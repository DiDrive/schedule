...
export default function ViewPage() {
  ...
  const [overflowDialogTitle, setOverflowDialogTitle] = useState('');
  const [overflowDialogTasks, setOverflowDialogTasks] = useState<Task[]>([]);
  ...
  const loadData = useCallback(async (showLoading: boolean) => {
    try {
      ...
      // 员工页只展示矩阵任务，避免“综合排期”覆盖该页面排期结果
      const matrixDbTasks = (data.matrixTasks && data.matrixTasks.length > 0)
        ? data.matrixTasks
        : (data.tasks || []).filter(dbTask => dbTask.task_source === 'matrix_view' || Boolean(dbTask.source_view_id));
      const convertedTasks: Task[] = matrixDbTasks.map(dbTask => ({
        id: dbTask.id,
        name: dbTask.name,
  ...
  const openOverflowDialog = useCallback((cellTasks: Task[], title: string) => {
    setOverflowDialogTasks(cellTasks);
    setOverflowDialogTitle(title);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <Dialog open={overflowDialogOpen} onOpenChange={setOverflowDialogOpen}>
          <DialogContent className="sm:max-w-[640px]">
            <DialogHeader>
              <DialogTitle>{overflowDialogTitle || '任务列表'}</DialogTitle>
            </DialogHeader>
            <div className="max-h-[60vh] overflow-y-auto space-y-2">
              {overflowDialogTasks.map((task, index) => (
                <button
                  key={`${getTaskUniqueKey(task)}-${index}`}
                  type="button"
                  className="w-full text-left rounded-md border bg-slate-50 px-3 py-2 hover:bg-slate-100 transition-colors"
                  onClick={() => {
                    setSelectedTask(task);
                    setTaskDialogOpen(true);
                ...
                >
                  ...
                  <div className="text-xs text-slate-500 mt-1">
                    负责人：{getTaskOwnerNames(task)}
                  </div>
                </button>
              ))}
              {overflowDialogTasks.length === 0 && (
                <div className="text-sm text-slate-500 text-center py-6">暂无任务</div>
              )}
            </div>
          </DialogContent>
        </Dialog>
