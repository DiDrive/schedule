...
### 2025-01-XX（第四次修复）：添加物料任务周末自动顺延功能

**问题描述**：
- 物料任务提供时间设置为2/1（周六）时，依赖任务开始时间计算错误
- 原因：2/1是周末，不是工作日，系统在计算依赖关系时识别不出
- 用户反馈：物料是周末提供的，需要自动顺延到最近的工作日

**根本原因**：
1. 物料任务提供时间是2/1（周六），在工作时间配置中不是工作日
2. 系统在计算依赖关系时，需要找到依赖任务的结束时间
3. 但由于2/1不是工作日，物料任务的处理逻辑存在问题
4. 导致依赖任务的开始时间计算不准确

**解决方案**：

#### 1. 添加物料任务周末自动顺延逻辑
在处理物料任务时，检查提供日期是否是工作日（周一至周五）：
- 如果是周日（dayOfWeek === 0），顺延1天到下周一
- 如果是周六（dayOfWeek === 6），顺延2天到下周一
- 保持用户设置的时间部分（时、分）不变
...
**技术细节**：
- 使用 `Date.getDay()` 获取星期几（0-6，0=周日，6=周六）
- 使用 `Date.setDate()` 顺延日期
- 顺延后的日期自动处理月份边界和闰年
- 保持原始时间部分（时、分、秒、毫秒）不变

**修改文件**：
- `src/lib/schedule-algorithms.ts`：添加物料任务周末自动顺延逻辑

**验收结果**：
- ✅ 物料任务提供时间是周末时，自动顺延到最近的工作日
- ✅ 周六顺延到下周一，周日顺延到下周一
- ✅ 保持用户设置的时间部分（时、分）不变
- ✅ 依赖任务必须在顺延后的物料可用时间之后才能开始
- ✅ 构建检查通过，无类型错误
- ✅ 服务运行正常

**示例场景**：
- 用户设置：物料提供时间 2/1（周六）10:00
- 系统处理：顺延到 2/3（周一）10:00
- 依赖任务：最早开始时间为 2/3（周一）10:00
